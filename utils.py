...
def load_data(file_name):
  ...
  with np.load(file_name) as data:
    ...
    #   mask = np.zeros((1, num_feature))
    #   for idx in range(np.shape(features)[2]):
    #       mask_valid = np.unique(np.where(features[:, :, idx] != -1)[1])
    #       mask[:, mask_valid] += 1
    #   temp = np.where(mask <= 2)[1]
...
def visualize_trajectory_2d(pose,x,y,path_name="Unknown",show_ori=False,path_save="Unknown"):
  '''
...
def visualize_trajectory_2d_pose(pose,path_name="Unknown",show_ori=False,path_save="Unknown"):
    '''
    function to visualize the trajectory in 2D
    Input:
        pose:   4*4*N matrix representing the camera pose, 
